...
int main(int argc, char **argv) {
  ...
  meeting.onData([](const uint8_t *data, size_t len, bool is_binary) {
    if (is_binary) {
      // Binary frame — hex preview (bytes may contain NULs).
      std::printf("\r[received data] %zu binary bytes:", len);
      for (size_t i = 0; i < len && i < 32; ++i)
        std::printf(" %02x", data[i]);
      std::printf("%s\n", len > 32 ? " ..." : "");
    } else {
      std::printf("\r[received data] %.*s\n", static_cast<int>(len),
    ...
    case 't': {
      ...
      if (std::fgets(line, sizeof(line), stdin)) {
        ...
        if (!msg.empty() && msg.back() == '\n')
          msg.pop_back();
        if (!msg.empty())
          std::printf(meeting.sendData(msg.data(), msg.size(), /*binary=*/false)
                          ? "  [sent text] %s\n"
                          : "  [send failed] %s\n",
                      msg.c_str());
      }
